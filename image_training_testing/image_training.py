...
# es = EarlyStopping(monitor='val_acc', mode='auto', verbose=1, patience=100)
# mc = ModelCheckpoint(args["model"], monitor='val_acc', mode='max', verbose=1, save_best_only=True)

# train the network
print("[INFO] training network...")
fit = model.fit(trainX, trainY, validation_data=(testX, testY), epochs=200, verbose=1)
# fit = model.fit_generator(aug.flow(trainX, trainY, batch_size=batch_size),validation_data=(testX, testY), steps_per_epoch=len(trainX) // batch_size,epochs=2000, verbose=1, callbacks=[es,mc])
